...
$(function() {
// Перехол к следующему полю номера карты
$(".next").keyup(function () {
...
// $('.cardn').on("keypress", function() {//При нажатии на клавиатуру
//     if (this.value.match(/[^0-9]/g)) {//Проверяет на какую кнопку нажал пользователь
//         this.value = this.value.replace(/[^0-9]/g, '');//Если if срабатывает, то символ выводится
//       }
//     });
...
$('.form_submit').click(function(){
   	...
   	if(v < d){
		...
		if($('.cardn').val() == '' || $('.card-name').val() == '' || $('.card-cvc').val() == '' || val == false){
			 
			 if($('.cardn').val() == ''){
			 	$('.cardn').addClass('error');
			 }
			 if($('.card-name').val() == ''){
			 ...
			 if(val == false){
			 	...
			 	$('.card-year').addClass('error');
			 }
			}	else {
				alert('Cmon! Cmon! Chop Chop!')
				// $.ajax({
    //         url: '/post.php',
    //         type: 'POST',
    //         data: {
